...
| Entregável | Detalhe |
|---|---|
| **13 agentes especializados** | `project-manager`, `tech-lead`, `product-owner`, `data-engineer`, `data-scientist`, `ml-engineer`, `ai-engineer`, `infra-devops`, `qa`, `researcher`, `security-auditor`, `design-engineer`, `marketing-strategist` — com organograma, cadeia de comando e protocolo de escalation |
...
| **`/update-memory`, `/update-memory-full`** | Registrar decisões de produto, restrições e entregáveis aprovados na memória persistente |
...
| **Memória persistente** | `project/memory/user_profile.md`, `project_genesis.md`, `project_history.md` e `MEMORY.md` criados na Fase 0 do `/kickoff` — somente project-manager e tech-lead leem antes de agir |
...
| **Gerador PDF/DOCX/PPTX** | `scripts/generate_docs.js` — MDs em `project/docs/` ganham contraparte em `project/docs/<sub>/generated/` via hook `post_write.sh` |
| **Versionamento de documentos** | Convenção `{nome}_YYYY-MM-DD_v{N}.md` em `archive/`; arquivos vigentes com nome estável — agentes nunca sobrescrevem versão anterior |
| **Convenção de commits** | Escopo `(system)` para infra agentic vs. `(project)` para trabalho de projeto, sem escopo para trabalho de produto |
| **Skills de domínio** | 21 skills enterprise em `.agents/skills/` (ai-engineering, backlog-management, ci-cd-setup, code-review, data-engineering, data-pipeline, frontend-engineering, frontend-patterns, go-to-market, infra-devops, llm-integration, market-research, ml-engineering, ml-experiment, product-management, project-kickoff, qa-testing, research-report, security-audit, security-review, testing-patterns) + caveman x3 opcional |
...
| **data-scientist** | tech-lead | data-engineer, researcher |
...
```text
.claude/
  agents/                    # agentes do template pai (tech-lead, template-coordinator)
  commands/
...
scripts/
  ...
  hooks/
    ...
    post_write.sh            # ruff/black em .py; gera docs em .md de project/docs/
  templates/
    ...
    agents/                  # 13 agentes copiados para .claude/agents/ do filho
    commands/
      ...
      update-memory-full.md  # /update-memory-full — atualização completa
      review.md / deploy.md / fix-issue.md / clean.md
.agents/
  skills/                    # 21 skills enterprise + caveman x3 (opcional)
.github/
  workflows/
